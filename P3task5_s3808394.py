...
def apply_tsne(X, n_components, perplexity, max_iter, init, random_state=2024):
    ...
    pipeline = make_pipeline(
        StandardScaler(),
        TSNE(n_components=n_components, perplexity=perplexity, n_iter=max_iter, init=init, random_state=random_state)
    )
...
def plot_tsne_projection(X_tsne_2d, color):
    ...
    plt.figure()
    plt.scatter(X_tsne_2d[:, 0], X_tsne_2d[:, 1], marker="o", c = color, cmap="viridis")
    plt.xlabel('X')
    plt.ylabel('Y')
    plt.show()
...
if __name__ == "__main__":
    ...
    X_tsne_2d = apply_tsne(X, n_components=2, perplexity=50, max_iter=2000, init="pca", random_state=2024)
